			...
			if($this->is_supervisor($id) || $this->is_assessor($id)){
				...
				////
					...
					foreach($this->get_members() as $member){
						$supervisor_input = (
							($this->is_supervisor($id)) ? 
								"<input type='number' name='supervisor[student][{$member->details->sim_id}]' min='0' max='{$this->marking_scheme->student->weight}' value='{$this->marking_scheme->student->individual->{$member->details->sim_id}}' />" : 
								$this->marking_scheme->student->individual->{$member->details->sim_id});
					...
					foreach($this->marking_scheme->feedback as $feedback_id => $feedback_details){
						$supervisor_input = (
							($this->is_supervisor($id)) ? 
